...
- `observability.py`: a failed result whose dict carries an int `status_code` is named from a fixed table - `upstream_http_400` / `401` / `403` / `404` / `422` / `429` / `500` / `502` / `503` / `504` for the statuses the API returns deliberately, `upstream_http_3xx` / `4xx` / `5xx` for the rest of the class. A code the tool named itself still wins (`agent_plane_unavailable` sits beside a 403). `mcp.success` follows `errors.is_error_payload`, the definition the tool protocol already applies, instead of a third private one: a partial envelope pairing an error note with data is a success, an error key with an empty or non-string value is a failure. `invalid_anchor`, `request_failed` and `missing_api_key`, which tools already returned, join the allowlist.
...
Full suite 595 passed; `ruff check` clean; `git diff --check` clean. New tests: 22 in `tests/test_observability.py` (status table, type strictness, precedence of a named code, table-to-allowlist drift guard, `is_error_payload` alignment from both sides, entity and keyless codes), 1 in `tests/test_client_errors.py` (3xx is a structured HTTP failure), 1 in `tests/test_gateway.py` (the delegated span names its operation). Every new test failed before the change and passes after it.
